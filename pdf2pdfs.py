...
DEFAULT_MODEL = "gemini-2.0-pro-exp-02-05"  # 支持视觉的模型
...
class GeminiPDFProcessor:
    ...
    def setup_api(self):
        ...
        try:
            ...
            # 网络连接失败提示
            if not self.proxy:
                logging.warning("您没有设置代理，如果在中国大陆访问 Google API 可能需要设置代理")
                logging.warning("可以通过 --proxy 参数设置代理，例如: --proxy http://localhost:7890")
        ...
        try:
            ...
            if "invalid_api_key" in str(e).lower():
                ...
                logging.error("获取密钥方法: 访问 https://makersuite.google.com/app/apikey 创建新API密钥")
                ...
                if not self.proxy:
                    logging.error("在中国大陆可能需要设置代理才能访问Google API")
            raise
    ...
    def process_pdf_folder(self, pdf_folder: str, output_folder: str, start_index: int = 0, 
        ...
        if not pdf_files:
            logging.error(f"在 {pdf_folder} 文件夹中未找到任何PDF文件!")
            return
